...
public class UnixAoutLoader extends AbstractProgramWrapperLoader {
	...
	/**
	 * Creates an AddressSpace and MemoryBlock for the .text section of the binary,
	 * reading its
	...
	/**
	 * Creates an AddressSpace and MemoryBlock for the .data section of the binary,
	 * reading its
	 * contents from the provider.
	 */
	protected void createDataSection(ByteProvider provider, TaskMonitor monitor, long size,
		...
		if (size > 0) {
			Address address = program.getAddressFactory().getDefaultAddressSpace().getAddress(addressFromHeader);
			try {
				InputStream stream = provider.getInputStream(fileOffset);
	...
	/**
	 * Creates a .bss section for this binary, ensuring that it has enough space to
	 * accommodate
	 * all of the symbols that are explictly assigned to .bss and also the symbols
	 * marked as
	...
	protected void createBssSection(long bssAddrVal) {
		...
		if (totalBssSize > 0) {
			Address bssAddr = this.program.getAddressFactory().getDefaultAddressSpace().getAddress(bssAddrVal);
			try {
	...
	/**
	 * Processes the binary's symbol table, taking one of four different actions
	 * depending on the
	 * symbol type:
	 * - N_TEXT are added to a list for disassembly later.
	 * - N_DATA are labeled immediately.
	 * - N_BSS are added to a list for labeling after the .bss section size has been
	 * established.
	 * - N_UNDF are added to a list for address assignment and labeling in .bss if
	 * the symbol is
	 * not found to already exist in a global symbol table (i.e. provided by another
	 * binary.)
	 */
	...
	/**
	 * Processes the text relocation table by fixing addresses based on the true
	 * location of each
	 * symbol.
	 */
			...
			if (relocationEntry.symbolNum < symTab.size()) {
				...
				if (relocationEntry.extern && this.textBlock.contains(relocAddr)) {
					...
					if (funcs.size() > 0) {
						...
						try {
							Address bssSymbolAddress = this.bssBlock.getStart().getAddressSpace()
									.getAddress(bssLocation);
	...
	/**
	 * Processes the data relocation table by fixing addresses based on the true
	 * location of each
	 * symbol.
	 */
	...
	/**
	 * Walks through the table of local function addresses, marks the locations as
	 * functions, and
